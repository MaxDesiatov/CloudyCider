...
final class EC2Store: BindableObject {
  ...
  func responseHandler(_ response: Result<EC2.DescribeInstancesResult, Error>) {
    do {
      guard let reservations = try response.get().reservations
      else { return }
      ...
      result = .success(nextStoreInstances)
    } catch {
      print(error)
